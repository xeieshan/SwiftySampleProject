...
/*
 Usage
 var segmentedCategories : SPSegmentControl!
 var dropDown : SPDropDownMenu!
 func setUpView() {
 segmentedCategories = SPSegmentControl()
 segmentedCategories?.segments = [
 SPTextSegment(text: "Candle"),
 SPTextSegment(text: "Football"),
 SPTextSegment(text: "New 2017"),
 SPTextSegment(text: "Early 2016"),
 SPTextSegment(text: "Entertainment & Arts"),
 SPTextSegment(text: "Finance"),
 SPTextSegment(text: "Video"),
 SPTextSegment(text: "Radio"),
 SPTextSegment(text: "Education"),
 SPTextSegment(text: "Sports"),
 SPTextSegment(text: "Weather"),
 SPTextSegment(text: "Headlines"),
 SPTextSegment(text: "Finance"),
 SPTextSegment(text: "Video"),
 SPTextSegment(text: "Radio"),
 SPTextSegment(text: "Education"),
 ...
 segmentedCategories?.style = .cover
 segmentedCategories?.delegate = self
 segmentedCategories?.selectedIndex = -1
 
 self.view.addSubview(segmentedCategories!)
 segmentedCategories?.translatesAutoresizingMaskIntoConstraints = false
 view.addConstraint(NSLayoutConstraint(item: segmentedCategories! as SPSegmentControl, attribute: .leading, relatedBy: .equal, toItem: self.view, attribute: .leading, multiplier: 1, constant: 20))
 view.addConstraint(NSLayoutConstraint(item: segmentedCategories! as SPSegmentControl, attribute: .trailing, relatedBy: .equal, toItem: self.view, attribute: .trailing, multiplier: 1, constant: 20))
 view.addConstraint(NSLayoutConstraint(item: segmentedCategories! as SPSegmentControl, attribute: .top, relatedBy: .equal, toItem: self.view, attribute: .top, multiplier: 1, constant: 10))
 view.addConstraint(NSLayoutConstraint(item: segmentedCategories! as SPSegmentControl, attribute: .height, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 0, constant: 40))
 @IBAction func btnSelect_Tags(sender: UIButton) {
 ...
 self.dropDown = SPDropDownMenu(sender, menuItems: temp, andMenuSelectedItems: [], shouldAllowMultipleSelection: true)
...
public protocol SPSegmentControlDelegate {
    func segmentControl(_ segmentControl: SPSegmentControl, selectIndex newIndex: Int, oldIndex: Int)
}

public protocol SPSegmentContentProtocol {
    var type: SPSegmentType { get }
}

public protocol SPTouchViewDelegate {
    func didTouch(_ location: CGPoint)
}

public class SPSegmentControl: UIControl {

    // MARK: Configuration - Content
    public var segments: [SPSegmentContentProtocol] = [] {
        didSet {
            innerSegments = segments.map({ (content) -> SPInnerSegment in
                return SPInnerSegment(content: content)
            })
            self.setNeedsLayout()
            if segments.count == 0 {
                selectedIndex = -1
    ...
    var innerSegments: [SPInnerSegment] = []

    // MARK: Configuration - Interaction
    public var delegate: SPSegmentControlDelegate?
    ...
    public var selectedSegment: SPSegmentContentProtocol? {
        return validIndex(selectedIndex) ? segments[selectedIndex] : nil
    }

    // MARK: Configuration - Appearance
    public var style: SPSegmentIndicatorStyle = .rainbow
    public var nonScrollDistributionStyle: SPSegmentNonScrollDistributionStyle = .left
    ...
    // Settings - Cover
    public typealias CoverRange = SPSegmentIndicatorRange

    public var cover_range: CoverRange = .segment
    public var cover_opacity: Float = 0.5
    public var cover_color: UIColor = UIColor.red

    // Settings - Strip
    public typealias StripRange = SPSegmentIndicatorRange
    public typealias StripLocation = SPSegmentIndicatorLocation
    ...
    // Settings - Rainbow
    public typealias RainbowLocation = SPSegmentIndicatorLocation
    ...
    // Settings - Arrow
    public typealias ArrowLocation = SPSegmentIndicatorLocation
    ...
    // Settings - ArrowStrip
    public typealias ArrowStripLocation = SPSegmentIndicatorLocation
    public typealias ArrowStripRange = SPSegmentIndicatorRange
    ...
    fileprivate var touchView: SPTouchView = SPTouchView()
    ...
    override public func layoutSubviews() {
        ...
        let initLayerCover = { [unowned self] in
            ...
            self.layerCover.cornerRadius = self.layerCover.frame.size.height/2
        ...
        let initLayerArrow = { [unowned self] (arrowFrame: CGRect, arrowLocation: SPSegmentIndicatorLocation, arrowColor: UIColor) in
    ...
    func segmentContentSize(_ segment: SPInnerSegment) -> CGSize {
    ...
    func indicatorStripFrame(_ index: Int, stripHeight: CGFloat, stripLocation: SPSegmentIndicatorLocation, stripRange: SPSegmentIndicatorRange) -> CGRect {
        if validIndex(index) {
    ...
    func indicatorArrowFrame(_ index: Int, arrowLocation: SPSegmentIndicatorLocation, arrowSize: CGSize) -> CGRect {
        if validIndex(index) {
...
extension SPSegmentControl: SPTouchViewDelegate {
...
class SPInnerSegment {
    ...
    let content: SPSegmentContentProtocol
    ...
    init(content: SPSegmentContentProtocol) {
        self.content = content
    }
}

public enum SPSegmentType {
    case text(String)
    case icon(UIImage)
}

public enum SPSegmentIndicatorStyle {
    case cover, strip, rainbow, arrow, arrowStrip
}

public enum SPSegmentIndicatorLocation {
    case up, down
}

public enum SPSegmentIndicatorRange {
    case content, segment
}

public enum SPSegmentNonScrollDistributionStyle {
    case center, left, right, average
}

class SPTouchView: UIView {

    var delegate: SPTouchViewDelegate?
